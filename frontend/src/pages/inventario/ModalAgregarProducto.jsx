      ...
      {seriales.map((serial) => {
        ...
        return (
          <div key={serial.id} className="w-full rounded-xl border border-gray-100 bg-gray-50 p-3 flex flex-col gap-1.5">
            ...
            <div className="flex justify-between text-sm"><span className="text-gray-500">Producto</span><span className="font-medium text-gray-900 text-right max-w-[60%]">{serial.producto_nombre}{serial.marca ? ` \xb7 ${serial.marca}` : ''}</span></div>
  ...
  return (
    <Modal open={open} onClose={onCancelar} title="" size="sm">
      <div className="flex flex-col items-center gap-4 py-2">
        ...
        <p className="text-sm text-center text-gray-700 leading-relaxed">\xbfDeseas <span className="font-semibold text-purple-700">reactivar estos seriales</span> en el inventario? Se marcar\xe1n como disponibles nuevamente.</p>
        <div className="flex gap-2 w-full">
          ...
          <Button className="flex-1 flex items-center justify-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white" onClick={onReactivar}><RefreshCw size={14} /> S\xed, reactivar</Button>
        </div>
      </div>
    </Modal>
  );
    ...
    <Modal open={open} onClose={onCerrar} title="" size="sm">
      <div className="flex flex-col items-center gap-4 py-2">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-1">{seriales.length === 1 ? 'Este equipo ya est\xe1 en inventario como disponible' : 'Estos equipos ya est\xe1n en inventario como disponibles'}</p>
        </div>
        <FilasSerial seriales={seriales} />
        <p className="text-sm text-center text-gray-700 leading-relaxed">No es necesario agregarlo nuevamente. Puedes continuar con los dem\xe1s IMEIs o cancelar.</p>
...
// ─── Buscador de cliente ───────────────────────────────────────────────────────
/**
 * Busca por nombre (\u226515 chars) o c\xe9dula (solo d\xeegitos, con Enter/bot\xf3n).
 * Permite crear un cliente nuevo con nombre, c\xe9dula y celular.
 */
function BuscadorCliente({ clienteSeleccionado, onClienteSeleccionado }) {
  ...
  const handleBuscarPorCedula = async () => {
    ...
    try {
      ...
      else setError('No se encontr\xf3 un cliente con esa c\xe9dula.');
    } catch { setError('No se encontr\xf3 un cliente con esa c\xe9dula.'); }
  ...
  const handleCrear = () => {
    ...
    if (!nuevoCliente.cedula.trim()) return setError('La c\xe9dula es requerida');
    mutCrear.mutate();
  };
      ...
      <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-500">CC {clienteSeleccionado.cedula}{clienteSeleccionado.celular ? ` \xb7 ${clienteSeleccionado.celular}` : ''}</p>
          </div>
        </div>
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-1.5">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por nombre o c\xe9dula..."
      ...
      {creandoNuevo && (
        <div className="bg-emerald-50 rounded-xl p-3 flex flex-col gap-2 border border-emerald-100">
          ...
          <div className="flex gap-2">
            <Input placeholder="C\xe9dula" value={nuevoCliente.cedula} onChange={(e) => setNuevoCliente({ ...nuevoCliente, cedula: e.target.value })} />
...
function PasoTipo({ onSelect }) {
  return (
    <div className="flex flex-col gap-3">
      <p className="text-sm text-gray-500">\xbfQu\xe9 tipo de producto vas a agregar?</p>
      <div className="flex gap-3">
        <button onClick={() => onSelect('serial')}
          ...
          <span className="text-xs text-gray-400 text-center">Proveedor \xb7 Celulares, tablets</span>
        </button>
        <button onClick={() => onSelect('cantidad')}
          ...
          <span className="text-xs text-gray-400 text-center">Proveedor \xb7 Accesorios</span>
        </button>
      </div>
...
function PasoCompraCliente({ sucursalKey, sucursalLista, onExito, onDuplicadosEncontrados }) {
  ...
  const mutCrearLinea = useMutation({
    mutationFn: () => crearProductoSerial({ nombre: nuevaLinea.nombre, marca: nuevaLinea.marca, modelo: nuevaLinea.modelo, precio: nuevaLinea.precio ? Number(nuevaLinea.precio) : null }),
    onSuccess: (res) => { queryClient.invalidateQueries({ queryKey: ['productos-serial'], exact: false }); setLineaSel(res.data.data); setCreandoLinea(false); setNuevaLinea({ nombre: '', marca: '', modelo: '', precio: '' }); },
    onError: (e) => setError(e.response?.data?.error || 'Error al crear l\xednea'),
  });

  const mutCrearProducto = useMutation({
    mutationFn: () => crearProductoCantidad({ nombre: nuevoProducto.nombre, unidad_medida: nuevoProducto.unidad_medida, costo_unitario: nuevoProducto.costo_unitario ? Number(nuevoProducto.costo_unitario) : null, precio: nuevoProducto.precio ? Number(nuevoProducto.precio) : null, cliente_origen: nombreCliente }),
    onSuccess: (res) => { queryClient.invalidateQueries({ queryKey: ['productos-cantidad'], exact: false }); setProductoSel(res.data.data); setCreandoProducto(false); setNuevoProducto({ nombre: '', unidad_medida: 'unidad', precio: '', costo_unitario: '' }); },
  ...
  const handleConfirmarSerial = async () => {
    ...
    if (!lineaSel)            return setError('Selecciona una l\xednea de producto');
    ...
    <div className="flex flex-col gap-4">
      ...
      {tipoProducto === 'serial' && (
        <div className="flex flex-col gap-3">
          {!lineaSel ? (
            <div className="flex flex-col gap-2">
              <p className="text-sm font-medium text-gray-700">L\xednea de producto</p>
              ...
              <button onClick={() => setCreandoLinea(!creandoLinea)} className="text-xs text-emerald-600 hover:text-emerald-700 font-medium text-left">+ Crear nueva l\xednea</button>
              {creandoLinea && (
                <div className="bg-emerald-50 rounded-xl p-3 flex flex-col gap-2 border border-emerald-100">
                  <p className="text-xs font-semibold text-emerald-700">Nueva l\xednea</p>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" className="flex-1" loading={mutCrearLinea.isPending} onClick={() => mutCrearLinea.mutate()}>Crear y seleccionar</Button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between bg-emerald-50 rounded-xl px-3 py-2">
                <div><p className="text-sm font-semibold text-emerald-800">{lineaSel.nombre}</p><p className="text-xs text-emerald-500">L\xednea seleccionada</p></div>
      ...
      {tipoProducto === 'cantidad' && (
        <div className="flex flex-col gap-3">
          {!productoSel ? (
              ...
              {creandoProducto && (
                <div className="bg-emerald-50 rounded-xl p-3 flex flex-col gap-2 border border-emerald-100">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" className="flex-1" loading={mutCrearProducto.isPending} onClick={() => mutCrearProducto.mutate()}>Crear y seleccionar</Button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between bg-emerald-50 rounded-xl px-3 py-2">
                <div><p className="text-sm font-semibold text-emerald-800">{productoSel.nombre}</p><p className="text-xs text-emerald-500">Stock actual: {productoSel.stock ?? 0}</p></div>
      ...
      {esAdmin && hayProducto && (
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 flex flex-col gap-1.5">
          ...
          <p className="text-xs text-emerald-600">Se registrar\xe1 en el costo del producto como referencia.</p>
        </div>
      )}
...
function PasoSerial({ sucursalKey, onExito, onDuplicadosEncontrados }) {
  ...
  const handleSeleccionarLinea = (producto) => { setLineaSel(producto); setError(''); if (esAdmin && producto.proveedor_id) setProveedorId(String(producto.proveedor_id)); };

  const mutCrearLinea = useMutation({
    mutationFn: () => crearProductoSerial({ nombre: nuevaLinea.nombre, marca: nuevaLinea.marca, modelo: nuevaLinea.modelo, precio: Number(nuevaLinea.precio), proveedor_id: proveedorId ? Number(proveedorId) : null }),
    onSuccess: (res) => { queryClient.invalidateQueries({ queryKey: ['productos-serial'], exact: false }); setLineaSel(res.data.data); setCreandoLinea(false); setNuevaLinea({ nombre: '', marca: '', modelo: '', precio: '' }); },
  ...
  const mutConfirmar = useMutation({
    mutationFn: async () => {
      ...
      if (proveedorId && modoPago) {
        ...
        if (esAdmin && proveedorId && !lineaSel.proveedor_id) await api.patch(`/productos/serial/${lineaSel.id}`, { proveedor_id: Number(proveedorId) });
      }
    },
    ...
    <div className="flex flex-col gap-4">
      {!lineaSel ? (
        <div className="flex flex-col gap-2">
          ...
          {creandoLinea && (
            <div className="bg-blue-50 rounded-xl p-3 flex flex-col gap-2">
              ...
              <Input id="sl-precio" type="number" placeholder="Precio de venta" value={nuevaLinea.precio} onChange={(e) => setNuevaLinea({ ...nuevaLinea, precio: e.target.value })} onKeyDown={(e) => e.key === 'Enter' && mutCrearLinea.mutate()} />
              <div className="flex gap-2">
                <Button variant="secondary" size="sm" className="flex-1" onClick={() => setCreandoLinea(false)}>Cancelar</Button>
                <Button size="sm" className="flex-1" loading={mutCrearLinea.isPending} onClick={() => mutCrearLinea.mutate()}>Crear y seleccionar</Button>
              </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {esAdmin && <InfoCompra proveedorId={proveedorId} setProveedorId={setProveedorId} costoCompra={costoCompra} setCostoCompra={setCostoCompra} modoPago={modoPago} setModoPago={setModoPago} />}
        </div>
      )}
      {error && <p className="text-sm text-red-500">{error}</p>}
...
function PasoCantidad({ sucursalKey, onExito }) {
  ...
  const handleSeleccionarProducto = (producto) => { setProductoSel(producto); setError(''); if (esAdmin) { if (producto.proveedor_id) setProveedorId(String(producto.proveedor_id)); if (producto.costo_unitario) setCostoCompra(String(producto.costo_unitario)); } };

  const mutCrear = useMutation({
    mutationFn: () => crearProductoCantidad({ nombre: nuevoProducto.nombre, unidad_medida: nuevoProducto.unidad_medida, costo_unitario: nuevoProducto.costo_unitario ? Number(nuevoProducto.costo_unitario) : null, precio: nuevoProducto.precio ? Number(nuevoProducto.precio) : null, proveedor_id: proveedorId ? Number(proveedorId) : null }),
  ...
  const mutConfirmar = useMutation({
    mutationFn: async () => {
      ...
      const costo = esAdmin ? (costoCompra ? Number(costoCompra) : null) : (productoSel?.costo_unitario || null);
      if (proveedorId && modoPago) {
        ...
        await ajustarStockCantidad(productoSel.id, { cantidad: cantidadNum, costo_unitario: costo, proveedor_id: esAdmin && proveedorId ? Number(proveedorId) : undefined });
      }
    },
      ...
      {!productoSel ? (
          ...
          {creandoNuevo && (
            <div className="bg-green-50 rounded-xl p-3 flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" className="flex-1" loading={mutCrear.isPending} onClick={() => mutCrear.mutate()}>Crear y seleccionar</Button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between bg-green-50 rounded-xl px-3 py-2">
            <div><p className="text-sm font-semibold text-green-800">{productoSel.nombre}</p><p className="text-xs text-green-500">Stock actual: {productoSel.stock ?? 0}</p></div>
  ...
  return (
    <>
      <Modal open onClose={onClose} title={tituloModal} size="md">
        ...
        {tipo && <button onClick={() => setTipo(null)} className="mt-3 text-xs text-gray-400 hover:text-gray-600 w-full text-center">\u2190 Volver a selecci\xf3n de tipo</button>}
      </Modal>
