// src/components/ui/ModalImprimirRecepcion.jsx
// Modal para elegir entre impresión POS (térmica) o PDF del recibo de recepción.

import { useState } from 'react';
import { Modal }    from './Modal';
import { Printer, FileDown, Share2, Loader2, X } from 'lucide-react';
import useExportarPdfRecepcion from '../../hooks/useExportarPdfRecepcion';
 ...
 * @param {Function} props.onImprimirPos   - Callback para abrir ReciboRecepcion (impresión POS)
 */
export function ModalImprimirRecepcion({ open, onClose, ordenId, numeroOrden, nombreCliente, onImprimirPos }) {
  const [formato, setFormato] = useState('pos');
  const [error,   setError]   = useState('');

  const { exportando, descargarPdf, compartirPdf, puedeCompartir } =
    useExportarPdfRecepcion();
  ...
  return (
    <Modal open={open} onClose={onClose} title="Recibo de recepción" size="sm">
      <div className="flex flex-col gap-5">

        <p className="text-sm text-gray-500 -mt-1">
          Orden <span className="font-semibold text-gray-700">{numOS}</span>
        ...
        {formato === 'pdf' && (
          <div className="flex flex-col gap-2">
            <p className="text-xs text-gray-400 bg-gray-50 rounded-xl px-3 py-2.5">
              Se descargará un PDF en formato A4 con el recibo de recepción del equipo.
            </p>
