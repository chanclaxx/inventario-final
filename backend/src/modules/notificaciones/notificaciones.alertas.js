...
const cartera = async (negocioId, sucursalId = null, diasAviso = null) => {
  ...
  try {
    ...
    const clasificar = (item) => {
      ...
      if (restantes < 0) {
        // Los días de atraso salen del calendario, no solo de la mora: sin mora
        // pactada `mora.dias_vencidos` es 0, y la lista del panel diría «hace 0
        // días» de algo que venció hace un mes.
        vencidos.push({
          ...item, estado: 'vencido', dias_restantes: 0,
          dias_vencidos: Math.max(item.dias_vencidos, -restantes),
        });
      } else {
...
// el deudor.
//
// Sale de `acreedores.service.getFacturasPorVencer` —la MISMA función que pinta
// Acreedores → Facturas, a donde lleva el aviso— y no de una consulta propia.
// Antes tenía la suya, contada por ORDEN y solo sobre órdenes 'Emitida': una
// compra suelta con plazo, o una orden ya cerrada con la factura sin pagar, no
// avisaban; y en modo 'recepcion' una orden con dos entregas eran UNA factura
// en el aviso y DOS en la pantalla. El usuario tocaba «2 facturas vencidas» y
// encontraba tres. Con la misma función, lo que dice el aviso es lo que se ve.
//
// Está detrás del flag `ordenes_compra_activas`: es el mismo que muestra la
// pestaña Facturas, y un aviso que lleva a una pestaña oculta no sirve.
//
...
const carteraProveedores = async (negocioId) => {
  ...
  try {
    ...
    const acreedoresService = require('../acreedores/acreedores.service');
    const { items, dias_aviso } = await acreedoresService.getFacturasPorVencer(negocioId);

    const forma = (f) => ({
      id:               f.cargo_id,
      acreedor_id:      f.acreedor_id,
      numero:           f.orden_numero ?? f.compra_numero ?? null,
    ...
    return {
      vencidas:   items.filter((f) => f.estado_pago === 'vencida').map(forma),
      por_vencer: items.filter((f) => f.estado_pago === 'por_vencer').map(forma),
      dias_aviso,
    };
  } catch (err) {
