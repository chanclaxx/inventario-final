...
const aprobarNegocio = async (negocioId) => {
  ...
  // Generar contraseña temporal para el admin_negocio
  ...
  try {
    ...
    // Activar negocio con plan trial por 15 días
    await client.query(
      `UPDATE negocios
       SET estado_plan = 'activo', fecha_vencimiento = NOW() + INTERVAL '15 days'
       WHERE id = $1`,
      [negocioId]
    );

    // Obtener la sucursal principal creada al registrarse
    const { rows: [sucursal] } = await client.query(
      `SELECT id FROM sucursales WHERE negocio_id = $1 ORDER BY id LIMIT 1`,
      [negocioId]
    );

    // Crear el usuario admin_negocio
    await client.query(
...
module.exports = { loginSuperadmin, getEstadisticas, getNegocios, aprobarNegocio, cambiarEstadoNegocio };