...
const _buildResumen = ({ pf, ac, ap, cp, aa, mn, rt, dv }) => {
  ...
  return {
    ...
    totales: {
      ...
      ingresos:      totalIngresos,  // neto = bruto - retomas
...
// ─── Helper: rango de la caja ─────────────────────────────────────────────────
// Inicio = fecha_apertura exacta. Fin = fecha_cierre si está cerrada, NOW() si sigue abierta.
...
const _getRangoCaja = async (cajaId) => {
  ...
  const fin    = caja.estado === 'Cerrada' && caja.fecha_cierre
    ? caja.fecha_cierre
    : new Date(); // NOW()
  return { inicio, fin };
};

// ─── getResumenDia — por sucursal, rango exacto de la caja ───────────────────
...
const getResumenDia = async (cajaId, sucursalId, negocioId) => {
  ...
  const [pagosFactura, abonosCredito, abonosPrestamo, compras, abonosAcreedor, manuales, devoluciones, retomas] =
    await Promise.all([
      ...
      // Abonos de créditos en el rango
      pool.query(`
      ...
      // Abonos de préstamos en el rango
      pool.query(`
      ...
      // Abonos a acreedores del negocio en el rango
      pool.query(`
      ...
      // Retomas de facturas en el rango — reducen el ingreso neto
      pool.query(`
...
// ─── getResumenGlobal — todas las sucursales, rango = hoy completo ────────────
// Vista admin: no hay "una caja" de referencia, se usa el día actual.
