...
const getSerialesTodas = async (negocioId) => {
  const { rows } = await pool.query(`
    ...
    ORDER BY suc.nombre, ps.nombre, s.vendido ASC, s.fecha_salida DESC
  `, [negocioId]);
  return rows;
};

module.exports = { getSeriales, getProductosCantidad, getVariantesPorSucursal, getSerialesTodas };